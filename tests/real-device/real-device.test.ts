...
test.afterAll(async () => {
  ...
  report.deviceCoverage.devices.forEach(device => {
    console.log(`  ${device.name}: ${device.count} tests (${device.percentage.toFixed(1)}%)`);
  });
  console.log('\n🖥️ OS Coverage:');
  report.osCoverage.operatingSystems.forEach(os => {
    console.log(`  ${os.name}: ${os.count} tests (${os.percentage.toFixed(1)}%)`);
  });
});
  ...
  for (const deviceConfig of criticalDevices) {
    test.describe(`${deviceConfig.deviceName} - ${deviceConfig.os} ${deviceConfig.osVersion}`, () => {
      ...
      test.afterEach(async ({ page }, testInfo) => {
        ...
        if (!passed && page) {
          await deviceManager.captureScreenshot(session.id, 'test-failure', 'error');
        }
      ...
      test('should complete farmer registration journey', async ({ page }) => {
        deviceManager.log(session.id, 'info', 'Starting farmer registration test');
        ...
        await deviceManager.captureScreenshot(session.id, 'registration-success');

        deviceManager.log(session.id, 'info', 'Farmer registration completed successfully');
      });

      test('should complete customer product search and add to cart', async ({ page }) => {
        ...
        deviceManager.log(session.id, 'info', 'Product search and cart test completed');
      });
...
test.describe('⚡ Real Device - Performance Tests', () => {
  const performanceDevices = DEVICE_CONFIGS.filter(
    d => d.deviceName.includes('iPhone 13') || d.deviceName.includes('OnePlus')
  );

  for (const deviceConfig of performanceDevices) {
    test(`${deviceConfig.deviceName} - Page load performance`, async ({ page }) => {
      ...
      try {
        ...
        deviceManager.log(session.id, 'error', 'Performance test failed', error);
    ...
    for (const deviceConfig of DEVICE_CONFIGS.slice(0, 5)) {
      ...
      try {
        ...
        const context = await browser.newContext({
          viewport: deviceConfig.deviceType === 'mobile' ? { width: 375, height: 667 } : null,
...
test.describe('🌾 Real Device - Agricultural Features', () => {
  ...
  for (const deviceConfig of farmDevices) {
    test(`${deviceConfig.deviceName} - Farm profile management`, async ({ page }) => {
      const session = await deviceManager.startSession(deviceConfig);
      ...
      try {
        ...
        deviceManager.log(session.id, 'error', 'Farm profile test failed', error);
        await deviceManager.endSession(session.id, false);
        throw error;
      }
    });

    test(`${deviceConfig.deviceName} - Seasonal product browsing`, async ({ page }) => {
      const session = await deviceManager.startSession(deviceConfig);

      try {
        deviceManager.log(session.id, 'info', 'Testing seasonal product features');
        ...
        deviceManager.log(session.id, 'info', 'Seasonal browsing test completed');
