...
const CATEGORIES = [
  ...
  'HERBS', 'GRAINS', 'HONEY', 'PRESERVES'
];

const SORT_OPTIONS = ['price_asc', 'price_desc', 'name_asc', 'name_desc', 'newest'];
...
function recordAPIMetrics(res, endpoint) {
  ...
  if (!success) {
    apiErrors.add(1);
    console.error(`❌ ${endpoint} failed: Status ${res.status}, Duration: ${res.timings.duration}ms`);
  }
...
function testFarmsSearch() {
  group('Farms Search', () => {
    const searchTerms = ['organic', 'fresh', 'local', 'biodynamic', 'sustainable'];
    const search = randomItem(searchTerms);

    const res = http.get(
      `${API_BASE}/farms?search=${search}`,
      { headers: { 'Accept': 'application/json' } }
    );
...
function testSearchAPI() {
  group('Search API', () => {
    const queries = [
      'tomato', 'lettuce', 'organic vegetables', 'fresh eggs',
      'local dairy', 'seasonal produce', 'honey', 'herbs'
    ];
