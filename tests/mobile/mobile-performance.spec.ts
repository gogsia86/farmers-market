...
test.describe('Mobile Performance Tests', () => {
  ...
  test.describe('Page Load Performance', () => {
    test('should load homepage quickly on mobile', async ({ page, context }) => {
    ...
    test('should load product pages efficiently on mobile', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should load farm profiles quickly on mobile', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should have fast Time to Interactive on mobile', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should measure JavaScript execution time', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
  ...
  test.describe('Resource Loading', () => {
    ...
    test('should use responsive images on mobile', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
      ...
      const responsiveImages = await page.evaluate(() => {
        const images = Array.from(document.querySelectorAll('img'));
        const withSrcset = images.filter(img => img.hasAttribute('srcset')).length;
        const withSizes = images.filter(img => img.hasAttribute('sizes')).length;
        const total = images.length;
      ...
      if (responsiveImages.total > 0) {
        expect(responsiveImages.withSrcset + responsiveImages.withSizes).toBeGreaterThan(0);
      }
    });
    ...
    test('should minimize CSS blocking time', async ({ page, context }) => {
      ...
      const cssStats = await page.evaluate(() => {
        ...
        const externalStyles = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).length;
      ...
      expect(cssStats.inlineStyles + cssStats.externalStyles).toBeGreaterThan(0);
    });
  });
  ...
  test.describe('Core Web Vitals', () => {
    test('should have good First Contentful Paint (FCP)', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
      ...
      const rating = perf.firstContentfulPaint < 1800 ? 'good' :
                     perf.firstContentfulPaint < 3000 ? 'acceptable' : 'poor';

      console.log('FCP:', `${perf.firstContentfulPaint}ms (${rating})`);
    });

    test('should have minimal Cumulative Layout Shift (CLS)', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should have fast Largest Contentful Paint (LCP)', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should have good First Input Delay (FID) proxy', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should have good Interaction to Next Paint (INP)', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
      ...
      if (interactions.length > 0) {
        const avgInteraction = interactions.reduce((a, b) => a + b) / interactions.length;
  ...
  test.describe('Agricultural Content Performance', () => {
    ...
    test('should handle image-heavy farm profiles', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
    ...
    test('should optimize seasonal product images', async ({ page, context }) => {
      const mobile = createMobileHelper(page, context);
      ...
      const imageOptimization = await page.evaluate(() => {
        ...
        const optimized = productImages.filter(img =>
          img.loading === 'lazy' ||
    ...
    test('should handle map loading efficiently', async ({ page, context }) => {
      ...
      if (await farmLink.count() > 0) {
        ...
        // Check if map exists
        const hasMap = await page.locator('[data-map], #map, .map-container').count() > 0;

        if (hasMap) {
          // Map should load without blocking main content
          const mainContentVisible = await page.evaluate(() => {
            const main = document.querySelector('main, [role="main"]');
            return main !== null && main.textContent !== null && main.textContent.length > 0;
          });
  ...
  test.describe('Device-Specific Performance', () => {
    ...
    for (const deviceName of testDevices) {
      test(`should perform well on ${deviceName}`, async ({ page, context }) => {
        const mobile = createMobileHelper(page, context);
