# 📚 Farmers Market Platform - Documentation Index

**Last Updated**: January 2025  
**Total Documentation**: 100+ files organized

---

## 🚀 Getting Started

- [**README**](../README.md) - Project overview
- [**Quick Start**](../QUICK_START.md) - Get started in 5 minutes
- [**Contributing**](../CONTRIBUTING.md) - How to contribute
- [**Current Status**](../STATUS.md) - Latest project status

---

## 📊 Current & Active Documentation

### 👨‍💻 For Developers
- [Developer Quick Reference](current/DEVELOPER_QUICK_REFERENCE.md)
- [Frontend Integration Guide](current/FRONTEND_INTEGRATION_GUIDE.md)
- [Integration Test Scenarios](current/INTEGRATION_TEST_SCENARIOS.md)
- [Technical Debt](current/TECHNICAL_DEBT.md)

### 🏗️ Architecture
- [Architecture Diagram](current/ARCHITECTURE_DIAGRAM.md)
- [Full Architecture Diagram](current/FULL_ARCHITECTURE_DIAGRAM.md)
- [Executive Summary](current/EXECUTIVE_SUMMARY.md)

### 🚀 Deployment
- [Deployment Readiness Checklist](current/DEPLOYMENT_READINESS_CHECKLIST.md)
- [Deployment Runbook](current/DEPLOYMENT_RUNBOOK.md)
- [Launch Day Runbook](current/LAUNCH_DAY_RUNBOOK.md)

---

## 📖 API Documentation

- [**API Reference**](api/API_REFERENCE.md)
- [**Getting Started with API**](api/GETTING_STARTED.md)
- [OpenAPI Spec (JSON)](api/openapi.json)
- [OpenAPI Spec (YAML)](api/openapi.yaml)
- [Postman Collection](api/postman-collection.json)
- [Swagger UI](api/index.html) - Open in browser

---

## 🚢 Deployment Guides

- [Docker Deployment](deployment/DOCKER_DEPLOYMENT.md)
- [Staging Deployment Quickstart](deployment/STAGING_DEPLOYMENT_QUICKSTART.md)
- [Vercel Deployment Guide](deployment/VERCEL_DEPLOYMENT_GUIDE.md)
- [Production Deployment](deployment/PRODUCTION_DEPLOYMENT_FINAL.md)
- [Deploy to Staging Now](deployment/DEPLOY_TO_STAGING_NOW.md)

---

## 📖 Guides & Tutorials

- [Refactoring Plan](guides/REFACTORING_PLAN.md)
- [Quick Reference](guides/REFACTORING_QUICK_REFERENCE.md)
- [Service Response Pattern](guides/SERVICE_RESPONSE_QUICK_REFERENCE.md)
- [Zod Migration Notes](guides/ZOD_MIGRATION_NOTES.md)
- [Continuation Guide](guides/CONTINUATION_GUIDE.md)

---

## 📦 Historical Archive

All historical reports preserved for reference:

- [**2024 Phase Reports**](archive/2024/phases/) - Development phases 1-5
- [**2024 Session Reports**](archive/2024/sessions/) - Daily session summaries
- [**2024 Completion Reports**](archive/2024/reports/) - Feature completion reports
- [**2025 Archive**](archive/2025/) - 2025 session reports

---

## 🔧 Quick Commands

### Development
\`\`\`bash
npm run dev              # Start development server
npm run build            # Build for production
npm run test             # Run tests
\`\`\`

### Docker
\`\`\`bash
npm run docker:up-dev    # Start development environment
npm run docker:logs      # View logs
npm run docker:down      # Stop containers
\`\`\`

### Database
\`\`\`bash
npm run db:migrate       # Run migrations
npm run db:seed          # Seed database
npm run db:studio        # Open Prisma Studio
\`\`\`

---

*Navigate documentation with agricultural consciousness* 🌾📚✨
