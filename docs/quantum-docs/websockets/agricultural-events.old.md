...
Event for subscribing to updates for one or more crops.
...
### Quantum-Aware Connection Setup
...
### Monitoring a Single Crop

```typescript
const wsUrl = process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:3000';
const ws = new WebSocket(`${wsUrl}/api/agricultural/monitor/${cropId}`);

ws.onmessage = (event) => {
  const message = JSON.parse(event.data) as WebSocketMessage<RealTimeCropUpdate>;
  if (message.type === 'CROP_UPDATE') {
    // Handle crop update
  }
};
```

### Monitoring Multiple Crops

```typescript
const wsUrl = process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:3000';
const ws = new WebSocket(`${wsUrl}/api/agricultural/monitor-multiple`);
