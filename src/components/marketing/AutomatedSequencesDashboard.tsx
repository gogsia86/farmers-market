...
import { PlayCircle, PauseCircle, Users, Mail, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Automated Email Sequences</h2>
          <p className="text-gray-600 mt-1">Set it and forget it - emails sent on autopilot</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard
          ...
          value={sequences.reduce((sum, s) => sum + s.stats.triggered, 0).toLocaleString()}
          change="+12%"
        />
        <StatCard
          icon={<Mail className="h-8 w-8 text-purple-600" />}
          label="Emails Sent"
          value={sequences.reduce((sum, s) => sum + s.stats.emailsSent, 0).toLocaleString()}
          change="+18%"
        />
        <StatCard
...
function SequenceCard({ sequence, onUpdate }: { sequence: EmailSequence; onUpdate: () => void }) {
      ...
      {sequence.stats.revenue > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          ...
          <div className="flex items-center justify-between mt-2">
            <span className="text-sm text-gray-600">Avg. Revenue per Email:</span>
...
function MetricBox({ label, value }: { label: string; value: number }) {
  return (
    <div className="text-center">
      <p className="text-2xl font-bold text-agricultural-600">{value.toLocaleString()}</p>
      <p className="text-sm text-gray-600">{label}</p>
    </div>
  );
}
...
function calculateConversionRate(sequences: EmailSequence[]): string {
  ...
  const totalConverted = sequences.reduce((sum, s) => sum + s.stats.converted, 0);
