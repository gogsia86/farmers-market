...
export function validateEnv(): Env {
  try {
    ...
    if (isBuildTime) {
      ...
      return envSchema.parse({
        ...
        STRIPE_PUBLISHABLE_KEY:
          process.env.STRIPE_PUBLISHABLE_KEY || "pk_placeholder",
    ...
    console.error(
      "🚨 PRODUCTION: Cannot start with invalid environment variables",
    );
    process.exit(1);
  }
}
...
export function logEnvironmentInfo(): void {
  if (isDevelopment()) {
    ...
    console.log(
      `🔐 Auth:             ${env.NEXTAUTH_SECRET ? "✅ Configured" : "❌ Not configured"}`,
    );
    console.log(
      `💳 Stripe:           ${env.STRIPE_SECRET_KEY ? "✅ Configured" : "❌ Not configured"}`,
    );
    console.log(
      `📧 Email (SendGrid): ${env.SENDGRID_API_KEY ? "✅ Configured" : "⚠️  Optional"}`,
    );
    console.log(
      `📊 Analytics:        ${env.NEXT_PUBLIC_GA_MEASUREMENT_ID ? "✅ Configured" : "⚠️  Optional"}`,
    );
    console.log(
      `🐛 Sentry:           ${env.SENTRY_DSN ? "✅ Configured" : "⚠️  Optional"}`,
    );
    console.log(
      `☁️  Cloudinary:       ${env.CLOUDINARY_API_KEY ? "✅ Configured" : "⚠️  Optional"}`,
    );
    console.log(
      `🗄️  Redis:            ${env.REDIS_URL || env.REDIS_HOST ? "✅ Configured" : "⚠️  Optional"}`,
    );
    console.log(
      `🤖 AI Features:      ${env.NEXT_PUBLIC_ENABLE_AI ? "✅ Enabled" : "❌ Disabled"}`,
    );
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
