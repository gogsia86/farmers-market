      ...
      tests: [
        {
          ...
          async run(page: Page) {
            ...
            const emptyState = await page
              .getByText(/no orders|no orders yet|start shopping/i)
              .count();
...

        {
          ...
          async run(page: Page) {
            ...
            const hasOrderNumber =
              (await firstOrder
                .locator('text=/order|#/i, [class*="order-number"]')
                .count()) > 0;
      ...
      tests: [
        {
          ...
          async run(page: Page) {
            ...
            const hasOrderNumber =
              (await page
                .locator('h1, h2, [data-testid="order-number"]')
                .count()) > 0;
            ...
            const hasTotal =
              (await page.getByText(/total|subtotal/i).count()) > 0;

            const hasStatus =
              (await page
                .getByText(/status|pending|processing|completed/i)
                .count()) > 0;
      ...
      tests: [
        {
          ...
          async run(page: Page) {
            ...
            const reorderButton = await page
              .locator(
