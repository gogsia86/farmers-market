...
export const DEFAULT_BOT_CONFIG: BotConfig = {
  ...
  version: '1.0.0',
  baseUrl: process.env.BASE_URL || process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3001',
...
export function loadConfigFromEnv(): Partial<BotConfig> {
  return {
    ...
    reporting: {
      ...
      formats: process.env.BOT_REPORT_FORMATS?.split(',') as any[] || ['json', 'console'],
 ...
 */
export function createConfig(preset?: string, overrides?: Partial<BotConfig>): BotConfig {
...
export function printConfig(config: BotConfig): void {
  ...
  console.log(`║ Headless: ${(config.browser.headless ? 'Yes' : 'No').padEnd(46)} ║`);
  console.log(`║ Parallel: ${(config.execution.parallel ? 'Yes' : 'No').padEnd(46)} ║`);
  console.log(`║ Max Concurrency: ${String(config.execution.maxConcurrency).padEnd(39)} ║`);
  console.log(`║ Retries: ${String(config.execution.retries).padEnd(47)} ║`);
  console.log('╠════════════════════════════════════════════════════════════╣');
  console.log(`║ Report Formats: ${config.reporting.formats.join(', ').padEnd(40)} ║`);
  console.log(`║ Output Dir: ${config.reporting.outputDir.padEnd(44)} ║`);
  console.log(`║ Screenshots: ${(config.reporting.screenshotOnFailure ? 'On Failure' : 'Disabled').padEnd(43)} ║`);
  console.log('╚════════════════════════════════════════════════════════════╝\n');
}
