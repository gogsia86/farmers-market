...
export class BotEngine {
  ...
  private eventHandlers: Map<EventType, Array<(event: BotEvent) => void>> =
    new Map();
  ...
  constructor(options: BotEngineOptions) {
    ...
    this.browserManager =
      options.browserManager ?? new BrowserManager(options.config);
  ...
  registerModule(module: BotModule): void {
    if (this.modules.has(module.id)) {
      logger.warn(
        `[BotEngine] Module ${module.id} already registered, overwriting`,
      );
    }
  ...
  registerSuite(suite: TestSuite): void {
    if (this.suites.has(suite.id)) {
      logger.warn(
        `[BotEngine] Suite ${suite.id} already registered, overwriting`,
      );
    }
  ...
  private async executeModuleWithRetry(
    ...
    const maxRetries =
      module.retryOnFailure !== false ? this.config.retryAttempts || 2 : 0;
    let lastError: Error | undefined;

    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      if (attempt > 0) {
        logger.info(
          `[BotEngine] Retrying module ${module.id} (attempt ${attempt}/${maxRetries})`,
        );
      ...
      try {
        ...
        logger.error(
          `[BotEngine] Module ${module.id} threw error:`,
          error as Error,
        );
      }
    }
  ...
  async startMonitoring(
    ...
    logger.info(
      `[BotEngine] Starting monitoring: ${suite.name} (interval: ${intervalSeconds}s)`,
    );
  ...
  private emit(type: EventType, data?: any): void {
    ...
    handlers.forEach((handler) => {
      try {
        ...
        logger.error(
          `[BotEngine] Event handler error for ${type}:`,
          error as Error,
        );
      }
    });
    ...
    wildcardHandlers.forEach((handler) => {
      try {
        ...
        logger.error(
          `[BotEngine] Wildcard event handler error:`,
          error as Error,
        );
      }
    });
