...
export class ProductService {
  ...
  static async createProduct(
    ...
    // 1. Verify farm ownership (using repository)
  ...
  static async getProductBySlug(
    ...
    // Use repository to search with farm slug filter
    const products = await productRepository.db.product.findFirst({
      where: {
        slug: productSlug,
        farm: {
          slug: farmSlug,
  ...
  static async updateProduct(
    ...
    if (updates.inventory) {
      ...
      const availableQuantity =
        (updates.inventory.quantity ?? existingInventory.quantity) -
        (updates.inventory.reservedQuantity ??
    ...
    if (updates.images) {
      primaryPhotoUrl = updates.images.find((img) => img.isPrimary)?.url;
    }
  ...
  static async batchUpdateProducts(
    ...
    for (const update of updates) {
      try {
        ...
        result.successful.push({ id: update.id, product });
        result.successCount++;
      } catch (error) {
        result.failed.push({
          id: update.id,
  ...
  private static async generateUniqueSlug(
    ...
    while (true) {
      ...
      const existing = await productRepository.db.product.findFirst({
        where: {
          slug,
          farmId,
  ...
  private static async validateProduct(
    ...
    // Price validation
    if (
      !productData.pricing?.basePrice?.amount ||
      productData.pricing.basePrice.amount <= 0
    ) {
      errors.push({
    ...
    if (
      productData.inventory?.quantity !== undefined &&
  ...
  static async incrementViewCount(productId: string): Promise<void> {
    // Use repository to update view count
    await productRepository.db.product.update({
      where: { id: productId },
      data: {
        viewsCount: {
          increment: 1,
