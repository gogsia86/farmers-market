...
export interface HarvestAnalytics {
  ...
  seasonalPerformance: Record<Season, {
    harvestCount: number;
    averageYield: number;
    averageQuality: number;
  }>;
...
export class HarvestTrackingService {
    ...
    try {
      ...
      if (!input.temperature || !input.humidity) {
        try {
          ...
          logger.warn('Failed to fetch weather data for harvest record', { error });
        }
      }
      ...
      const harvestRecord = await database.harvestRecord.create({
        data: {
          ...
          gradeSplit: input.gradeSplit as Prisma.JsonValue,
      ...
      if (input.predictionId) {
        await this.updatePredictionWithActuals(input.predictionId, harvestRecord);
      }
      ...
      this.generateMLTrainingData(harvestRecord).catch(error => {
        logger.error('Failed to generate ML training data', { error, harvestRecordId: harvestRecord.id });
      });
  ...
  async getHarvestAnalytics(
    ...
    const totalYield = harvests.reduce((sum, h) => sum + Number(h.totalYield), 0);
    ...
    // Top crops by yield
    const cropYields = new Map<string, {
      cropId: string;
      cropName: string;
      totalYield: number;
      qualities: number[];
    ...
    const topCrops = Array.from(cropYields.values())
      .map(c => ({
        ...
        averageQuality: c.qualities.reduce((a, b) => a + b, 0) / c.qualities.length,
    ...
    const seasonalPerformance: Record<Season, {
      harvestCount: number;
      averageYield: number;
      averageQuality: number;
    }> = {
    ...
    const seasonalData = new Map<Season, { yields: number[]; qualities: number[] }>();
    ...
    seasonalData.forEach((data, season) => {
      seasonalPerformance[season] = {
        ...
        averageYield: data.yields.reduce((a, b) => a + b, 0) / data.yields.length,
        averageQuality: data.qualities.reduce((a, b) => a + b, 0) / data.qualities.length
      };
    });
    ...
    const averageError = totalPredictions > 0 ? totalError / totalPredictions : 0;
    const rmse = totalPredictions > 0
      ? Math.sqrt(sumSquaredError / totalPredictions)
      : 0;

    // Revenue metrics
    const totalRevenue = harvests.reduce((sum, h) =>
      sum + (h.marketValue ? Number(h.marketValue) : 0), 0
    );
    const totalCost = harvests.reduce((sum, h) =>
      sum + (h.totalCost ? Number(h.totalCost) : 0), 0
    );
  ...
  async createYieldPrediction(
    ...
    try {
      ...
      const daysToMaturity = crop.daysToMaturity as { min: number; max: number };
      ...
      if (historicalHarvests.length > 0) {
        const avgHistoricalYield = historicalHarvests.reduce((sum, h) =>
          sum + Number(h.yieldPerArea || 0), 0
  ...
  async getHarvestInsights(farmId: string): Promise<HarvestInsights> {
    ...
    harvests.forEach(h => {
      ...
      if (existing) {
        existing.avgQuality = (existing.avgQuality * existing.count + qualityScore) / (existing.count + 1);
        existing.avgYield = (existing.avgYield * existing.count + Number(h.totalYield)) / (existing.count + 1);
        existing.count++;
      } else {
    ...
    const bestPerformingCrops = Array.from(cropPerformance.entries())
      .sort((a, b) => (b[1].avgQuality + b[1].avgYield / 1000) - (a[1].avgQuality + a[1].avgYield / 1000))
      .slice(0, 5)
      .map(([_, crop]) => crop.name);

    // Optimal harvest windows
    const harvestWindows = new Map<string, { quality: number; count: number }>();
    ...
    harvests.forEach(h => {
      ...
      if (existing) {
        existing.quality = (existing.quality * existing.count + qualityScore) / (existing.count + 1);
        existing.count++;
      } else {
    ...
    const diseaseRate = harvests.filter(h => h.diseasePresent).length / harvests.length;
    const pestRate = harvests.filter(h => h.pestDamage).length / harvests.length;

    if (diseaseRate > 0.2) {
      improvementRecommendations.push('High disease incidence detected. Consider crop rotation and disease-resistant varieties.');
    }

    if (pestRate > 0.2) {
      improvementRecommendations.push('Significant pest damage detected. Implement integrated pest management strategies.');
    }

    // Check soil health
    const lowPHHarvests = harvests.filter(h => h.soilPH && Number(h.soilPH) < 6.0);
    if (lowPHHarvests.length > harvests.length * 0.3) {
      improvementRecommendations.push('Soil pH appears low. Consider lime application to improve soil health.');
    }
    ...
    const poorQualityRate = harvests.filter(h => h.quality === 'POOR' || h.quality === 'FAILED').length / harvests.length;
    if (poorQualityRate > 0.2) riskFactors.push('Quality issues affecting more than 20% of harvests');
  ...
  private async generateMLTrainingData(harvest: HarvestRecord): Promise<void> {
    try {
      ...
      const normalizedYield = Math.min(Number(harvest.yieldPerArea || harvest.totalYield) / 10000, 1);
      const qualityScore = this.qualityToScore(harvest.quality) / 5;

      await database.mLTrainingData.create({
        data: {
          harvestRecordId: harvest.id,
          avgTemperature: harvest.temperature ? Number(harvest.temperature) / 120 : 0.5,
          totalRainfall: harvest.rainfall7Days ? Number(harvest.rainfall7Days) / 10 : 0.5,
          ...
          soilNitrogenLevel: harvest.soilNitrogen ? Number(harvest.soilNitrogen) / 200 : 0.5,
          soilPhosphorusLevel: harvest.soilPhosphorus ? Number(harvest.soilPhosphorus) / 100 : 0.5,
          soilPotassiumLevel: harvest.soilPotassium ? Number(harvest.soilPotassium) / 400 : 0.5,
          soilPHLevel: harvest.soilPH ? (Number(harvest.soilPH) - 4) / 6 : 0.5,
          soilMoistureAvg: harvest.soilMoisture ? Number(harvest.soilMoisture) / 100 : 0.5,
      ...
      logger.debug('ML training data generated', { harvestRecordId: harvest.id });
    } catch (error) {
      logger.error('Failed to generate ML training data', { error, harvestId: harvest.id });
    }
  }
  ...
  private calculateWeatherScore(weather: any): number {
    ...
    const tempScore = weather.temperature >= 60 && weather.temperature <= 80 ? 1.0 : 0.7;

    // Humidity ideal: 50-70%
    const humidityScore = weather.humidity >= 50 && weather.humidity <= 70 ? 1.0 : 0.7;
