...
export function useAnalytics() {
  ...
  const trackViewFarm = useCallback(
    (farm: { id: string; name: string; type?: string }) => {
  ...
  const trackSeasonalFilter = useCallback(
    (season: string, resultCount: number) => {
  ...
  const trackOrderStatusCheck = useCallback(
  ...
  const trackError = useCallback(
    (error: { message: string; page?: string; fatal?: boolean }) => {
