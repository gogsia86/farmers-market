...
import type { PlantingCalendarResponse } from "@/types/biodynamic.types";
      ...
      case "current": {
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            lunarPhaseEmoji: biodynamicCalendar.getLunarPhaseEmoji(
              context.lunarPhase,
            ),
...
function getSeasonalGuidance(season: Season): string {
  const guidance: Record<Season, string> = {
    SPRING:
      "Spring is the season of renewal and growth. Focus on planting new crops, preparing soil, and establishing your growing plan for the year.",
    SUMMER:
      "Summer brings peak growth and activity. Maintain consistent watering, monitor for pests, and begin harvesting early crops.",
    ...
    WINTER:
      "Winter is a time of rest and planning. Review your growing season, plan crop rotations, maintain equipment, and prepare for spring.",
  };
...
function getSeasonalOperationGuidance(
  ...
  const descriptions = operations
    .map(
      (op) =>
        `• ${op}: ${operationDescriptions[op] || "Important seasonal task"}`,
    )
    .join("\n");
