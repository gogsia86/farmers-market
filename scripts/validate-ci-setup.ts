...
class CIValidator {
  ...
  async validateWorkflowFile(): Promise<void> {
    ...
    if (!exists) {
      this.addResult({
        ...
        details: 'The UBF workflow file is missing. It should have been created in Phase 4.',
      });
      return;
    }
    ...
    try {
      ...
      if (!content.includes('name:') || !content.includes('on:') || !content.includes('jobs:')) {
        this.addResult({
          ...
          details: 'The workflow file is missing required keys (name, on, jobs)',
        });
        return;
      }
      ...
      const missingJobs = requiredJobs.filter(job => !content.includes(`${job}:`));
  ...
  async validateGitHubActionsEnabled(): Promise<void> {
    ...
    try {
      const { stdout } = await execAsync('gh repo view --json hasIssuesEnabled');
  ...
  async validateDatabaseConnection(): Promise<void> {
    ...
    this.addResult({
      ...
      details: 'Ensure TEST_DATABASE_URL secret is set in GitHub and points to a valid PostgreSQL database',
    });
  }
  ...
  async validateUBFFramework(): Promise<void> {
    ...
    const runnerExists = await fileExists('src/lib/testing/core/test-runner.ts');
    ...
    if (cliExists && engineExists && runnerExists) {
      ...
      this.addResult({
        ...
        details: 'Core UBF files not found. The framework may not be properly installed.',
      });
    }
  }
  ...
  private generateReport(): ValidationReport {
    ...
    for (const category of categories) {
      ...
      for (const result of categoryResults) {
        ...
        if (result.status === 'FAIL' || result.status === 'WARN') {
          recommendations.push(`[${result.status}] ${result.check}: ${result.message}`);
        }
    ...
    console.log(colorize(`⚠️  Warnings:   ${summary.warnings}`, summary.warnings > 0 ? 'yellow' : 'green'));
    ...
    if (summary.failed === 0) {
      ...
      console.log(colorize(`   Fix ${summary.failed} failed check(s) before enabling workflow`, 'red'));
    }
