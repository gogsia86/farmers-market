...
async function checkEndpoint(path: string, retries = CONFIG.retries): Promise<CheckResult> {
...
async function performHealthCheck(): Promise<MonitorReport> {
  ...
  const criticalFailed = checks
    .filter((c) => CONFIG.criticalPages.includes(c.url.replace(CONFIG.baseUrl, '')))
  ...
  log(`║  Success Rate: ${((passed / checks.length) * 100).toFixed(1)}%${' '.repeat(38)} ║`, 'cyan');
  log(`║  Avg Response: ${formatTime(avgResponseTime).padEnd(41)} ║`, 'cyan');
  log('╚════════════════════════════════════════════════════════════╝\n', 'cyan');

  // Display overall status
  const statusColor = overall === 'healthy' ? 'green' : overall === 'degraded' ? 'yellow' : 'red';
...
async function showHelp() {
  ...
  log('  npm run monitor:list         - List all monitored endpoints\n', 'cyan');
  ...
  log('  BASE_URL  - Override base URL (default: http://localhost:3001)', 'cyan');
  log('  Example: BASE_URL=https://staging.example.com npm run monitor\n', 'cyan');
  ...
  log('  BASE_URL=https://staging.farmersmarket.com npm run monitor:all\n', 'cyan');
}
...
async function checkCriticalOnly() {
  ...
  log(`\n📊 Results: ${passed}/${checks.length} passed`, passed === checks.length ? 'green' : 'red');
...
async function runWorkflowValidation() {
  ...
  log('\n╔════════════════════════════════════════════════════════════╗', 'cyan');
  log(`║  Success Rate: ${successRate.toFixed(1)}% (${passed}/${results.length})${' '.repeat(28)} ║`, 'cyan');
  log('╚════════════════════════════════════════════════════════════╝\n', 'cyan');
...
async function main() {
  ...
  try {
    ...
    log(`\n❌ Monitor Error: ${error instanceof Error ? error.message : 'Unknown error'}`, 'red');
    process.exit(1);
  }
}
