...
function printResult(result: TestResult) {
  ...
  if (result.success && result.data) {
    ...
    if (data) {
      ...
      if (data.executions) {
        console.log(
          `   Executions: ${data.executions.length} returned`,
        );
      }
      if (data.alerts) {
        console.log(
      ...
      if (data.metrics) {
        console.log(
          `   Metrics: ${data.metrics.length} data points`,
        );
      }
    }
  }
...
async function checkDatabaseHealth(): Promise<boolean> {
  ...
  try {
    ...
    const [executions, healthChecks, notifications] =
      await Promise.all([
