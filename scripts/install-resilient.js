...
function logProgress(current, total, packageName) {
  ...
  process.stdout.write(
    `\r${colors.cyan}[${bar}]${colors.reset} ${percent}% (${current}/${total}) ${packageName}${colors.reset}`,
  );
}
...
function installBatch(packages, batchNumber, totalBatches, progress) {
  ...
  log(`\n\n${"=".repeat(70)}`, "cyan");
  log(
    `📦 Batch ${batchNumber}/${totalBatches} (${packages.length} packages)`,
    "bright",
  );
  log("=".repeat(70), "cyan");
...
function retryFailed(progress) {
  ...
  for (const pkg of progress.failed) {
    try {
      ...
      execSync(
        `npm install ${pkg.name}@${pkg.version} --legacy-peer-deps --no-save`,
        {
          cwd: ROOT_DIR,
          stdio: "inherit",
...
function printSummary(progress) {
  ...
  if (progress.failed.length === 0) {
    ...
    log(
      `   3. Commit lockfile: git add package-lock.json && git commit`,
      "blue",
    );
  } else {
    ...
    progress.failed.forEach((pkg) => {
      log(
        `      npm install ${pkg.name}@${pkg.version} --legacy-peer-deps`,
        "blue",
      );
    });
  }
...
function main() {
  ...
  const remaining = packages.filter(
    (p) => !progress.installed.includes(p.name),
  );
