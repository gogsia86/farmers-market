const markdownpdf = require("markdown-pdf");
const fs = require("fs");
const path = require("path");

console.log("🚀 Starting PDF conversion...");
console.log("═══════════════════════════════════════════════════════════════");

const inputFile = path.join(__dirname, "../FULL_ARCHITECTURE_DIAGRAM.md");
const outputFile = path.join(__dirname, "../FULL_ARCHITECTURE_DIAGRAM.pdf");

// Check if input file exists
if (!fs.existsSync(inputFile)) {
  console.error("❌ ERROR: Input file not found!");
  console.error(`   Looking for: ${inputFile}`);
  process.exit(1);
}

console.log(`📄 Input:  ${path.basename(inputFile)}`);
console.log(`📄 Output: ${path.basename(outputFile)}`);
console.log("═══════════════════════════════════════════════════════════════");

const options = {
  paperFormat: "A4",
  paperOrientation: "portrait",
  paperBorder: {
    top: "2cm",
    right: "1.5cm",
    bottom: "2cm",
    left: "1.5cm",
  },
  renderDelay: 1000,
  // Custom CSS for better styling
  cssPath: path.join(__dirname, "pdf-styles.css"),
};

// Create CSS file if it doesn't exist
const cssPath = path.join(__dirname, "pdf-styles.css");
if (!fs.existsSync(cssPath)) {
  const defaultCSS = `
body {
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif;
  font-size: 11pt;
  line-height: 1.6;
  color: #333;
  max-width: 100%;
  margin: 0 auto;
}

h1 {
  color: #2c3e50;
  border-bottom: 3px solid #4CAF50;
  padding-bottom: 10px;
  margin-top: 30px;
  font-size: 24pt;
  page-break-before: auto;
}

h1:first-of-type {
  page-break-before: avoid;
  margin-top: 0;
}

h2 {
  color: #34495e;
  border-bottom: 2px solid #ddd;
  padding-bottom: 8px;
  margin-top: 25px;
  font-size: 18pt;
  page-break-after: avoid;
}

h3 {
  color: #4CAF50;
  margin-top: 20px;
  font-size: 14pt;
}

h4 {
  color: #666;
  margin-top: 15px;
  font-size: 12pt;
}

code {
  background-color: #f4f4f4;
  border: 1px solid #ddd;
  border-radius: 3px;
  padding: 2px 6px;
  font-family: 'Courier New', Consolas, monospace;
  font-size: 10pt;
  color: #c7254e;
}

pre {
  background-color: #f8f8f8;
  border: 1px solid #ddd;
  border-radius: 5px;
  padding: 15px;
  overflow-x: auto;
  margin: 15px 0;
  page-break-inside: avoid;
}

pre code {
  background-color: transparent;
  border: none;
  padding: 0;
  color: #333;
}

blockquote {
  border-left: 4px solid #4CAF50;
  padding-left: 15px;
  color: #666;
  font-style: italic;
  margin: 15px 0;
}

table {
  border-collapse: collapse;
  width: 100%;
  margin: 15px 0;
  page-break-inside: avoid;
}

table th {
  background-color: #4CAF50;
  color: white;
  font-weight: bold;
  padding: 12px;
  text-align: left;
  border: 1px solid #ddd;
}

table td {
  padding: 10px;
  border: 1px solid #ddd;
}

table tr:nth-child(even) {
  background-color: #f9f9f9;
}

ul, ol {
  margin: 10px 0;
  padding-left: 30px;
}

li {
  margin: 5px 0;
}

a {
  color: #2196F3;
  text-decoration: none;
}

a:hover {
  text-decoration: underline;
}

hr {
  border: none;
  border-top: 2px solid #ddd;
  margin: 30px 0;
}

/* Mermaid diagram placeholder */
.language-mermaid {
  background-color: #e8f5e9;
  border: 2px dashed #4CAF50;
  padding: 20px;
  text-align: center;
  font-style: italic;
  color: #666;
  page-break-inside: avoid;
}

/* Badges and special elements */
.badge {
  display: inline-block;
  padding: 3px 8px;
  border-radius: 3px;
  font-size: 9pt;
  font-weight: bold;
}

/* Print-specific styles */
@page {
  margin: 2cm 1.5cm;
}

@media print {
  h1, h2, h3 {
    page-break-after: avoid;
  }

  pre, table, img {
    page-break-inside: avoid;
  }

  a[href]:after {
    content: " (" attr(href) ")";
    font-size: 90%;
    color: #666;
  }
}

/* Status badges */
.status-active {
  background-color: #4CAF50;
  color: white;
}

.status-pending {
  background-color: #FF9800;
  color: white;
}

.status-error {
  background-color: #F44336;
  color: white;
}
`;

  fs.writeFileSync(cssPath, defaultCSS);
  console.log("✨ Created default CSS styling file");
}

console.log("⏳ Converting markdown to PDF...");
console.log("   (This may take 30-60 seconds for large documents)");
console.log("");

markdownpdf(options)
  .from(inputFile)
  .to(outputFile, (err) => {
    if (err) {
      console.error(
        "═══════════════════════════════════════════════════════════════",
      );
      console.error("❌ CONVERSION FAILED!");
      console.error(
        "═══════════════════════════════════════════════════════════════",
      );
      console.error(`Error: ${err.message}`);
      console.error("");
      console.error("💡 Troubleshooting tips:");
      console.error("   1. Make sure the input file exists");
      console.error("   2. Check that you have write permissions");
      console.error("   3. Close the PDF if it's already open");
      console.error("   4. Try running: npm install markdown-pdf --save-dev");
      console.error("");
      process.exit(1);
    }

    console.log(
      "═══════════════════════════════════════════════════════════════",
    );
    console.log("✅ PDF CREATED SUCCESSFULLY!");
    console.log(
      "═══════════════════════════════════════════════════════════════",
    );
    console.log(`📁 Location: ${outputFile}`);

    // Get file size
    const stats = fs.statSync(outputFile);
    const fileSizeInMB = (stats.size / (1024 * 1024)).toFixed(2);
    console.log(`📊 File Size: ${fileSizeInMB} MB`);
    console.log("");
    console.log("📖 Note: Mermaid diagrams will appear as code blocks.");
    console.log(
      "   For rendered diagrams, use Method 1 from EXPORT_TO_PDF_INSTRUCTIONS.md",
    );
    console.log("");
    console.log("🎉 Done! You can now open the PDF file.");
    console.log(
      "═══════════════════════════════════════════════════════════════",
    );
  });
