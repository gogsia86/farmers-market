...
class ErrorDetector {
  ...
  async detectDuplicateFiles(): Promise<void> {
    ...
    for (const file of files) {
      try {
        ...
        if (fileHashes.has(hash)) {
          ...
          const similarity = this.calculateSimilarity(content, fs.readFileSync(duplicate, 'utf8'));
    ...
    console.log(`   Found ${this.errors.duplicateFiles.length} duplicate file pairs`);
  }
  ...
  async detectImportConflicts(): Promise<void> {
    ...
    for (const file of sourceFiles) {
      try {
        ...
        Object.entries(importPatterns).forEach(([module, config]) => {
          ...
          const uniquePaths = [...new Set(foundImports.map(imp => {
            const match = imp.match(/from\s+['"]([^'"]+)['"]/);
            return match ? match[1] : '';
          }))].filter(Boolean);

          if (uniquePaths.length > 1 || (uniquePaths.length === 1 && !uniquePaths[0].includes(config.canonical))) {
            const hasCanonical = uniquePaths.some(p => p.includes(config.canonical));
            if (!hasCanonical) {
    ...
    console.log(`   Found ${this.errors.importConflicts.length} import conflicts`);
  }
  ...
  async detectTypeConflicts(): Promise<void> {
    ...
    for (const typeName of commonTypes) {
      try {
        ...
        const definitions: Array<{ file: string; definition: string; line?: number }> = [];
        ...
        for (const file of files) {
          ...
          lines.forEach((line, index) => {
            const typeRegex = new RegExp(`(type|interface|class)\\s+${typeName}\\b`, 'i');
  ...
  async detectAPIInconsistencies(): Promise<void> {
    ...
    const apiEndpoints = fs.readdirSync(apiDir, { withFileTypes: true })
    ...
    console.log(`   Found ${this.errors.apiInconsistencies.length} API inconsistencies`);
  }
  ...
  async detectServiceDuplications(): Promise<void> {
    ...
    for (let i = 0; i < serviceNames.length; i++) {
      for (let j = i + 1; j < serviceNames.length; j++) {
        ...
        if (similarity > 0.7 && name1 !== name2) {
          this.errors.serviceDuplications.push({
            service: `${name1} / ${name2}`,
            files: [...(serviceGroups[name1] || []), ...(serviceGroups[name2] || [])],
    ...
    console.log(`   Found ${this.errors.serviceDuplications.length} service duplication issues`);
  }
  ...
  async detectCanonicalImportViolations(): Promise<void> {
    ...
    console.log(`   Found ${this.errors.canonicalImportViolations.length} canonical import violations`);
  }
  ...
  async detectMissingExports(): Promise<void> {
    ...
    const servicesIndexPath = path.join(this.rootDir, 'src/lib/services/index.ts');
    if (!fs.existsSync(servicesIndexPath)) {
      ...
      for (const serviceFile of serviceFiles) {
        ...
        const expectedExport = new RegExp(`(export.*${serviceName}|from.*${serviceName})`, 'i');
    ...
    console.log(`   Found ${this.errors.missingExports.length} missing exports`);
  }
  ...
  async detectBuildErrors(): Promise<void> {
    ...
    try {
      ...
      console.log(`   Found ${errorLines.length} TypeScript errors (showing first 20)`);
    }
  }
  ...
  private generateErrorReport(): void {
    ...
    const totalErrors = this.errors.duplicateFiles.length +
                        this.errors.importConflicts.length +
                        this.errors.typeConflicts.length +
                        this.errors.apiInconsistencies.filter(i => i.severity === 'error').length +
                        this.errors.serviceDuplications.length +
                        this.errors.buildErrors.length +
                        this.errors.canonicalImportViolations.length;

    const totalWarnings = this.errors.apiInconsistencies.filter(i => i.severity === 'warning').length +
                          this.errors.missingExports.length;
    ...
    if (this.errors.duplicateFiles.length > 0) {
      ...
      this.errors.duplicateFiles.forEach((dup, i) => {
        console.log(`  ${i + 1}. Similarity: ${(dup.similarity * 100).toFixed(0)}%`);
    ...
    if (this.errors.importConflicts.length > 0) {
      ...
      if (this.errors.importConflicts.length > 10) {
        console.log(`  ... and ${this.errors.importConflicts.length - 10} more conflicts`);
      }
    }
    ...
    if (this.errors.typeConflicts.length > 0) {
      ...
      this.errors.typeConflicts.forEach((conflict, i) => {
        console.log(`  ${i + 1}. Type: ${conflict.type} (${conflict.definitions.length} definitions)`);
    ...
    // API Inconsistencies
    const criticalAPI = this.errors.apiInconsistencies.filter(i => i.severity === 'error');
    ...
    if (this.errors.canonicalImportViolations.length > 0) {
      console.log('\n❌ CANONICAL IMPORT VIOLATIONS:');
      this.errors.canonicalImportViolations.slice(0, 5).forEach((violation, i) => {
      ...
      if (this.errors.canonicalImportViolations.length > 5) {
        console.log(`  ... and ${this.errors.canonicalImportViolations.length - 5} more violations`);
      }
    }
    ...
    if (this.errors.buildErrors.length > 0) {
      ...
      if (this.errors.buildErrors.length > 5) {
        console.log(`  ... and ${this.errors.buildErrors.length - 5} more errors`);
      }
    }
  }
  ...
  private saveErrorReport(): void {
    ...
    const report = {
      ...
      summary: {
        totalErrors: this.errors.duplicateFiles.length +
                     this.errors.importConflicts.length +
                     this.errors.typeConflicts.length +
                     this.errors.apiInconsistencies.filter(i => i.severity === 'error').length +
                     this.errors.serviceDuplications.length +
                     this.errors.buildErrors.length +
                     this.errors.canonicalImportViolations.length,
        totalWarnings: this.errors.apiInconsistencies.filter(i => i.severity === 'warning').length +
                       this.errors.missingExports.length,
        byCategory: {
          ...
          canonicalImportViolations: this.errors.canonicalImportViolations.length,
