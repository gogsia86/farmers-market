...
function logTest(name: string, status: 'pass' | 'fail' | 'warn' | 'skip', duration?: number) {
...
class TestRunner {
  ...
  async runTest(
    name: string,
    category: string,
    testFn: () => Promise<{ status: 'pass' | 'fail' | 'warn' | 'skip'; message?: string; details?: any; suggestions?: string[] }>
  ): Promise<void> {
  ...
  async testBasicConnectivity(): Promise<any> {
    try {
      ...
      return {
        status: 'fail',
        message: error.message,
        suggestions: ['Check if Vercel deployment is live', 'Verify DNS configuration'],
      };
    }
  }
    ...
    try {
      ...
      if (responseTime < 500) {
        ...
        return {
          ...
          suggestions: ['Consider optimizing server-side rendering', 'Check database query performance'],
        };
      } else {
        return {
          status: 'fail',
          message: `Too slow (${responseTime}ms)`,
          suggestions: ['Investigate server performance', 'Check for blocking operations'],
        };
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          ...
          suggestions: ['Consider adding a /api/health endpoint for monitoring'],
        };
      } else {
        return {
          status: 'fail',
  ...
  async testAuthPages(): Promise<any> {
    try {
      ...
      if (loginResponse.status === 200) {
        const hasLoginForm = loginResponse.body.includes('login') ||
        ...
        return {
          ...
          suggestions: ['Check routing configuration', 'Verify login page exists'],
        };
      } else {
        return {
          status: 'fail',
  ...
  async testAPIRoutes(): Promise<any> {
    ...
    for (const route of routes) {
      try {
        const response = await makeRequest(`${baseUrl}${route}`, { timeout: 5000 });
    ...
    if (passCount === routes.length) {
      return { status: 'pass', message: `All ${routes.length} API routes accessible` };
    } else if (passCount > 0) {
      return {
        status: 'warn',
      ...
      return {
        ...
        details: results,
        suggestions: ['Check API route configuration', 'Verify build completed successfully'],
      };
    }
  }
  ...
  async testStaticAssets(): Promise<any> {
    ...
    for (const asset of assets) {
      try {
        const response = await makeRequest(`${baseUrl}${asset}`, { timeout: 5000 });
  ...
  async testDatabaseConnectivity(): Promise<any> {
    try {
      ...
      return {
        ...
        suggestions: ['Check API endpoints return data', 'Verify database connection in Vercel'],
      };
    } catch (error: any) {
      return {
        status: 'fail',
        message: error.message,
        suggestions: ['Check DATABASE_URL environment variable', 'Verify Prisma connection'],
      };
    }
  }
  ...
  async testErrorHandling(): Promise<any> {
    try {
      const response = await makeRequest(`${baseUrl}/this-page-does-not-exist-12345`);

      if (response.status === 404) {
        const has404Page = response.body.includes('404') ||
  ...
  async testSecurityHeaders(): Promise<any> {
    try {
      ...
      const presentHeaders = Object.entries(securityHeaders).filter(([_, value]) => value !== undefined);
      const missingHeaders = Object.entries(securityHeaders).filter(([_, value]) => value === undefined);
  ...
  async testPerformanceMetrics(): Promise<any> {
    ...
    if (avgTime < 500 && maxTime < 1000) {
      ...
      return {
        ...
        suggestions: ['Consider performance optimization', 'Check for blocking operations'],
      };
    } else {
      return {
        status: 'fail',
    ...
    try {
      ...
      if (passedChecks >= 3) {
        ...
        return {
          ...
          details: checks,
          suggestions: ['Add missing meta tags', 'Implement proper SEO structure'],
        };
      } else {
        return {
          status: 'warn',
  ...
  async testMobileResponsiveness(): Promise<any> {
    try {
      ...
      if (response.status === 200) {
        ...
        const hasResponsiveClasses = /responsive|container|flex|grid/i.test(html);
        ...
        if (hasViewport && hasResponsiveClasses) {
          ...
          return {
            ...
            suggestions: ['Test on actual mobile devices', 'Use Chrome DevTools mobile emulation'],
          };
        } else {
          return {
            status: 'warn',
        ...
        return { status: 'fail', message: `Mobile request returned ${response.status}` };
      }
    } catch (error: any) {
      return { status: 'fail', message: error.message };
    }
    ...
    try {
      ...
      if (corsHeader === '*') {
        return {
          ...
          suggestions: ['Consider restricting CORS to specific origins for production'],
        };
      } else if (corsHeader) {
        return {
          status: 'pass',
  ...
  generateReport(): TestReport {
    ...
    const slowest = responseTimes.reduce(
      (max, r) => (r.duration > max.time ? { url: r.name, time: r.duration } : max),
      { url: 'N/A', time: 0 }
    );

    const fastest = responseTimes.reduce(
      (min, r) => (r.duration < min.time ? { url: r.name, time: r.duration } : min),
    ...
    return {
      ...
      performance: {
        ...
        fastestEndpoint: fastest.time !== Infinity ? fastest : { url: 'N/A', time: 0 },
      },
    };
  }
  ...
  printReport(report: TestReport) {
    ...
    const ratingColor =
      parseFloat(successRate) >= 90 ? 'green' : parseFloat(successRate) >= 70 ? 'yellow' : 'red';
    log(`🎯 Success Rate: ${successRate}%`, ratingColor);
    console.log('');

    if (report.performance.avgResponseTime > 0) {
      log('⚡ Performance:', 'bright');
      log(`  Average Response: ${report.performance.avgResponseTime}ms`, 'reset');
      log(`  Fastest: ${report.performance.fastestEndpoint.url} (${report.performance.fastestEndpoint.time}ms)`, 'green');
      log(`  Slowest: ${report.performance.slowestEndpoint.url} (${report.performance.slowestEndpoint.time}ms)`, 'yellow');
      console.log('');
    }
...
async function main() {
  console.log('\n' + '╔' + '═'.repeat(58) + '╗');
  log('║' + ' '.repeat(58) + '║', 'cyan');
  log('║' + '     🤖 VERCEL DEPLOYMENT TESTING BOT 🤖     '.padEnd(59) + '║', 'cyan');
  log('║' + ' '.repeat(58) + '║', 'cyan');
  console.log('╚' + '═'.repeat(58) + '╝\n');

  log(`🎯 Target: ${baseUrl}`, 'bright');
  log(`🔧 Mode: ${isQuickMode ? 'Quick' : isFullMode ? 'Full' : 'Standard'}`, 'bright');
  ...
  await runner.runTest('Basic Connectivity', 'connectivity', () => runner.testBasicConnectivity());
  await runner.runTest('SSL/TLS Certificate', 'security', () => runner.testSSLCertificate());
  await runner.runTest('Response Time', 'performance', () => runner.testResponseTime());

  // Health & API Tests
  logSection('🏥 HEALTH & API TESTS');
  await runner.runTest('Health Endpoint', 'api', () => runner.testHealthEndpoint());
  await runner.runTest('API Routes', 'api', () => runner.testAPIRoutes());
  await runner.runTest('Database Connectivity', 'database', () => runner.testDatabaseConnectivity());

  // Page Tests
  logSection('📄 PAGE TESTS');
  await runner.runTest('Authentication Pages', 'pages', () => runner.testAuthPages());
  await runner.runTest('Error Handling', 'pages', () => runner.testErrorHandling());
  await runner.runTest('Static Assets', 'assets', () => runner.testStaticAssets());

  // Security Tests
  logSection('🔒 SECURITY TESTS');
  await runner.runTest('Security Headers', 'security', () => runner.testSecurityHeaders());
  await runner.runTest('CORS Configuration', 'security', () => runner.testCORSConfiguration());

  // Performance & SEO Tests (skip in quick mode)
  if (!isQuickMode) {
    logSection('⚡ PERFORMANCE TESTS');
    await runner.runTest('Performance Metrics', 'performance', () => runner.testPerformanceMetrics());

    logSection('🔍 SEO & UX TESTS');
    await runner.runTest('SEO Basics', 'seo', () => runner.testSEOBasics());
    await runner.runTest('Mobile Responsiveness', 'mobile', () => runner.testMobileResponsiveness());
  }
