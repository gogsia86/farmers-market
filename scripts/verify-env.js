...
console.log(`${colors.cyan}🔍 Checking environment: ${colors.bright}${environment}${colors.reset}\n`);
...
if (existingEnvFiles.length === 0) {
  ...
  console.log(`${colors.yellow}💡 Create a .env.local file with your environment variables${colors.reset}\n`);
} else {
  console.log(`${colors.green}✅ Found env files: ${existingEnvFiles.join(', ')}${colors.reset}\n`);
}

// Validate required variables
console.log(`${colors.bright}${colors.magenta}📋 REQUIRED VARIABLES${colors.reset}\n`);
...
Object.entries(requiredVariables).forEach(([category, variables]) => {
  ...
  variables.forEach(varName => {
    ...
    if (!value) {
      ...
      const displayValue = isSensitive ? '***' + value.slice(-4) : value.slice(0, 50);
...
console.log(`${colors.bright}${colors.magenta}📋 OPTIONAL VARIABLES${colors.reset}\n`);
...
if (environment === 'production') {
  console.log(`${colors.bright}${colors.magenta}🔒 PRODUCTION SECURITY CHECKS${colors.reset}\n`);

  productionOnlyVariables.forEach(varName => {
    const value = process.env[varName];

    if (!value) {
      console.log(`  ${colors.red}❌ ${varName}${colors.reset} - CRITICAL: Required in production!`);
      missingRequired.push(varName);
    } else if (value.includes('localhost') || value.includes('127.0.0.1')) {
      warnings.push(`${varName} contains localhost - should use production URL`);
      console.log(`  ${colors.red}⚠️  ${varName}${colors.reset} - WARNING: Contains localhost!`);
    } else if (varName === 'NEXTAUTH_SECRET' && value === 'development-secret-change-me') {
      warnings.push(`${varName} is using default development value`);
      console.log(`  ${colors.red}⚠️  ${varName}${colors.reset} - WARNING: Using default value!`);
    } else {
      console.log(`  ${colors.green}✅ ${varName}${colors.reset} - Production ready`);
    }
  });
...
console.log(`${colors.bright}${colors.magenta}🌾 AGRICULTURAL CONSCIOUSNESS CHECK${colors.reset}\n`);
...
const consciousnessScore = agriculturalVars.filter(v => process.env[v]).length / agriculturalVars.length * 100;

if (consciousnessScore === 100) {
  console.log(`  ${colors.green}✅ Agricultural Consciousness: MAXIMUM (${consciousnessScore}%)${colors.reset}`);
} else if (consciousnessScore >= 66) {
  console.log(`  ${colors.yellow}⚠️  Agricultural Consciousness: MODERATE (${consciousnessScore}%)${colors.reset}`);
} else {
  console.log(`  ${colors.red}❌ Agricultural Consciousness: LOW (${consciousnessScore}%)${colors.reset}`);
}
...
console.log(`  ${colors.green}✅ Valid variables: ${validCount}${colors.reset}`);
console.log(`  ${colors.red}❌ Missing required: ${missingRequired.length}${colors.reset}`);
console.log(`  ${colors.yellow}⚠️  Missing optional: ${missingOptional.length}${colors.reset}`);
...
if (missingRequired.length > 0) {
  console.log(`${colors.bright}${colors.red}❌ MISSING REQUIRED VARIABLES:${colors.reset}\n`);
  missingRequired.forEach(varName => {
    console.log(`  ${colors.red}• ${varName}${colors.reset}`);
  });
  console.log('');
  console.log(`${colors.yellow}💡 Add these variables to your .env.local file${colors.reset}\n`);
}

// Display missing optional
if (missingOptional.length > 0 && environment === 'production') {
  console.log(`${colors.bright}${colors.yellow}⚠️  MISSING OPTIONAL VARIABLES (Recommended for production):${colors.reset}\n`);
  missingOptional.slice(0, 5).forEach(varName => {
    console.log(`  ${colors.yellow}• ${varName}${colors.reset}`);
  });
  if (missingOptional.length > 5) {
    console.log(`  ${colors.yellow}• ... and ${missingOptional.length - 5} more${colors.reset}`);
  }
  console.log('');
}
...
if (missingRequired.length === 0 && warnings.length === 0) {
  ...
  console.log(`║  ${colors.green}🌾 Divine agricultural consciousness: ACTIVE${colors.reset}               ║`);
  console.log(`║  ${colors.green}⚡ Ready for deployment!${colors.reset}                                   ║`);
} else if (missingRequired.length === 0 && warnings.length > 0) {
  console.log(`║  ${colors.yellow}⚠️  ENVIRONMENT VERIFICATION PASSED WITH WARNINGS${colors.reset}          ║`);
  ...
  console.log(`║  ${colors.red}🚫 Missing required variables - cannot deploy${colors.reset}               ║`);
}
...
if (missingRequired.length > 0) {
  console.log(`${colors.red}${colors.bright}Exiting with error code 1${colors.reset}\n`);
  process.exit(1);
} else if (warnings.length > 0 && environment === 'production') {
  console.log(`${colors.yellow}${colors.bright}Warnings detected - review before production deployment${colors.reset}\n`);
