...
async function waitForServer(url: string, maxRetries: number): Promise<boolean> {
  ...
  for (let i = 1; i <= maxRetries; i++) {
    ...
    if (i < maxRetries) {
      log(`⏳ Server not ready yet (attempt ${i}/${maxRetries})... retrying in ${RETRY_DELAY / 1000}s`, colors.yellow);
...
async function main() {
  ...
  if (!serverRunning) {
    ...
    log(`  5. Come back to this terminal and run this script again\n`, colors.cyan);
