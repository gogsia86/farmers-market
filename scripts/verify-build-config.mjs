...
function verifyPackageJson() {
  ...
  try {
    ...
    const hasPostinstall = packageJson.scripts?.postinstall?.includes('prisma generate');
    check(
...
function verifyNextConfig() {
  ...
  try {
    ...
    const hasDeprecatedTurbo = /experimental:\s*{[^}]*turbo:\s*{/.test(configContent);
    check(
    ...
    const hasDeprecatedDisableLogger = /disableLogger:\s*(true|false)/.test(configContent);
    check(
    ...
    const hasModernTreeshake = /webpack:\s*{[^}]*treeshake:\s*{[^}]*removeDebugLogging/.test(configContent);
    check(
    ...
    const hasModernReactAnnotation = /webpack:\s*{[^}]*reactComponentAnnotation:\s*{/.test(configContent);
    check(
    ...
    const hasSourceMaps = /productionBrowserSourceMaps:\s*true/.test(configContent);
    check(
    ...
    const hasSecurityHeaders = /X-Frame-Options|X-Content-Type-Options/.test(configContent);
    check(
...
function verifySentryConfig() {
  ...
  try {
    ...
    const hasSentryProject = /project:\s*["']farmers-market-prod["']/.test(configContent);
    check(
      'Sentry project configured',
      hasSentryProject,
      hasSentryProject ? 'project: farmers-market-prod' : 'Sentry project not found'
    );
    ...
    const hasWidenClientFileUpload = /widenClientFileUpload:\s*true/.test(configContent);
    check(
    ...
    const hasTunnelRoute = /tunnelRoute:\s*["']\/monitoring["']/.test(configContent);
    warn(
...
function verifyBuildExpectations() {
  ...
  print('     → This is NORMAL for *_client-reference-manifest.js files', 'cyan');
  print('     → They don\'t contain code, just React Server Component metadata', 'cyan');
  print('     → Your actual page chunks (.js files) DO have source maps', 'cyan');
...
function printSummary() {
  ...
  print(`Failed: ${failedChecks}`, failedChecks > 0 ? 'red' : 'green', failedChecks > 0 ? icons.error : icons.success);
  print(`Warnings: ${warnings}`, 'yellow', warnings > 0 ? icons.warning : icons.success);

  console.log('');

  if (failedChecks === 0) {
    print('✨ ALL CHECKS PASSED! ✨', 'green', icons.success);
    print('Your build configuration is optimized and ready for production.', 'cyan');
    console.log('');
    print('Next steps:', 'bright');
    print('  1. Commit changes: git add -A && git commit -m "fix(build): optimize Vercel config"', 'cyan');
    ...
    print('Please review the failed checks above and fix the issues.', 'yellow');
    console.log('');
