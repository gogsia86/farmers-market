...
function validateNpmrc() {
  ...
  if (config["legacy-peer-deps"] !== undefined) {
    ...
    if (config["legacy-peer-deps"] === "true") {
      ...
      logWarning(
        "legacy-peer-deps is disabled - may cause peer dependency conflicts",
      );
      warnings++;
    }
  } else {
...
function crossValidate(npmrcConfig, vercelJson) {
  ...
  const vercelUsesLegacyFlag =
    vercelJson.installCommand?.includes("--legacy-peer-deps");

  if (npmrcLegacyPeerDeps === "false" && vercelUsesLegacyFlag) {
    logError(
      "CONFLICT: .npmrc disables legacy-peer-deps but vercel.json enables it",
    );
    logError("The .npmrc setting will override the command-line flag!");
    errors++;
  } else if (npmrcLegacyPeerDeps === "true" && vercelUsesLegacyFlag) {
    logSuccess(
      ".npmrc and vercel.json are aligned (both use legacy-peer-deps)",
    );
...
function validatePackageLock() {
  ...
  if (!fs.existsSync(packageLockPath)) {
    logWarning(
      "package-lock.json not found (will be generated on first install)",
    );
    warnings++;
    return;
  }
