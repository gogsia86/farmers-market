...
const checkForIssues = () => {
  ...
  files.forEach(file => {
    try {
      ...
      const oldPatterns = [
        { pattern: 'getInitialProps', reason: 'Deprecated in Next.js 13+ App Router' },
        { pattern: 'getServerSideProps', reason: 'Use Server Components in App Router' },
        { pattern: 'getStaticProps', reason: 'Use Server Components in App Router' },
        { pattern: 'getStaticPaths', reason: 'Use generateStaticParams in App Router' },
        { pattern: 'class.*extends.*Component', reason: 'Consider using function components' },
        { pattern: 'componentDidMount', reason: 'Use useEffect hook instead' },
        { pattern: 'componentWillMount', reason: 'Deprecated React lifecycle method' },
      ...
      const hasServerOnlyFeatures = content.includes('prisma') ||
      ...
      const isComponent = !relativePath.includes('page.tsx') &&
  ...
  filesByName.forEach((paths, filename) => {
    if (paths.length > 1) {
      issues.duplicateNames[filename] = paths.map(p => path.relative(srcPath, p));
    }
  });
  ...
  filesByCaseInsensitiveName.forEach((paths, lowerName) => {
    if (paths.length > 1) {
      ...
      if (uniqueNames.size > 1) {
        issues.caseInsensitiveDuplicates[lowerName] = paths.map(p => path.relative(srcPath, p));
      }
    }
  });
    ...
    function findRoutes(dir, basePath = '') {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const newBasePath = entry.name.startsWith('(') && entry.name.endsWith(')')
            ? basePath
          ...
          routeFiles.push({ route: basePath || '/', file: path.relative(appPath, fullPath) });
        }
      }
    }
    findRoutes(appPath);
...
const formatIssues = (issues) => {
  ...
  if (Object.keys(issues.caseInsensitiveDuplicates).length > 0) {
    ...
    Object.entries(issues.caseInsensitiveDuplicates).forEach(([name, paths]) => {
      console.log(`\n   ${name}:`);
      paths.forEach(p => console.log(`      - ${p}`));
    });
    console.log();
  ...
  // Summary
  console.log('📊 SUMMARY:');
  console.log(`   Total files scanned: ${issues.largeFiles.length + issues.emptyFiles.length + issues.missingExports.length + issues.oldPatterns.length}`);
  console.log(`   Duplicate filenames: ${Object.keys(issues.duplicateNames).length}`);
  console.log(`   Case-insensitive duplicates: ${Object.keys(issues.caseInsensitiveDuplicates).length}`);
  console.log(`   Old patterns: ${issues.oldPatterns.length}`);
  console.log(`   Route conflicts: ${issues.routeConflicts.length}`);
  console.log(`   Improper client components: ${issues.improperClientComponents.length}`);
...
try {
  ...
  const hasIssues = Object.keys(issues.duplicateNames).length > 0 ||
