...
function handleList(type?: string) {
  ...
  if (!type || type === "suites") {
    ...
    console.log(
      "  health, checker    - Health checks and monitoring (16 checks)",
    );
    console.log("");
  }
...
function handleConfig(preset?: string) {
  ...
  console.log(
    `  TEST_USER_PASSWORD: ${process.env.TEST_USER_PASSWORD ? "***" : "(not set)"}`,
  );
...
function handleHelp(command?: string) {
  if (command && COMMANDS[command as keyof typeof COMMANDS]) {
    ...
    Object.entries(COMMANDS).forEach(([name, cmd]) => {
      console.log(
        `  ${colors.cyan}${name.padEnd(12)}${colors.reset} ${cmd.description}`,
      );
    });
    console.log("");
    ...
    console.log(
      "  --preset=NAME        Use configuration preset (quick, mvp, debug, etc.)",
    );
