...
class DeploymentHealthMonitor {
  ...
  async checkHealth(): Promise<boolean> {
    ...
    try {
      ...
      if (process.argv.includes('--full')) {
        this.log('\n  🔄 Running full build test (this may take 3-5 minutes)...', 'yellow');
    ...
    if (criticalFailures > 0) {
      ...
      this.log('   Fix critical issues before deploying to production', 'yellow');
      this.log('', 'reset');
      return false;
    } else if (warnings > 0) {
      ...
      this.log('   🎯 Pattern matches proven success baseline (7+ builds)', 'green');
  ...
  private async checkNodeVersion(): Promise<HealthCheckResult> {
    try {
      ...
      return {
        ...
        detail: isCompatible ? 'Vercel uses Node.js 20.x or 24.x' : 'Upgrade to Node.js 20.x or higher',
        critical: false
      };
    } catch (error) {
      return {
  ...
  private async checkLockfileCompatibility(): Promise<HealthCheckResult> {
    try {
      ...
      await execAsync('npx --package=node@24 --package=npm@latest -- npm ls --json', {
        timeout: 30000
      });
      ...
      return {
        ...
        detail: 'Fix: npx --package=node@24 --package=npm@latest -- npm install --legacy-peer-deps',
        critical: true
      };
    }
  }
  ...
  private async checkPackageCount(): Promise<HealthCheckResult> {
    try {
      const { stdout } = await execAsync('npm ls --json --all 2>/dev/null || echo "{}"');
      const data = JSON.parse(stdout || '{}');
      const count = this.countPackages(data);

      const diff = Math.abs(count - this.EXPECTED_METRICS.packageCount);
      const withinTolerance = diff <= this.EXPECTED_METRICS.packageCountTolerance;
  ...
  private async checkPrismaMigrations(): Promise<HealthCheckResult> {
    try {
      const { stdout } = await execAsync('npx prisma migrate status 2>&1 || true', {
        timeout: 30000
      });

      const hasUnapplied = stdout.includes('not yet been applied') ||
  ...
  private printHistoricalComparison(): void {
    ...
    const successRate = (recentBuilds.filter(b => b.success).length / recentBuilds.length) * 100;

    this.log(`  Last ${recentBuilds.length} builds:`, 'bright');
    this.log(`    Success Rate: ${successRate.toFixed(1)}%`, 'white');

    if (recentBuilds.length > 0) {
      const avgBuildTime = recentBuilds
      ...
      const avgPackages = recentBuilds
        .reduce((sum, b) => sum + b.packageCount, 0) / recentBuilds.length;
      ...
      if (uniqueCounts.size === 1) {
        ...
        this.log(`    Consistency: ⚠️  ${uniqueCounts.size} different configurations`, 'yellow');
      }
    }
  ...
  async generateReport(): Promise<void> {
    ...
    this.log(`\n📊 Last 10 Deployments:\n`, 'cyan');
    this.log(`  Success Rate: ${(recentSuccess / recent.length * 100).toFixed(2)}%`, 'white');
