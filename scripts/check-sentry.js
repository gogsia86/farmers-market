...
console.log(`${colors.bright}${colors.cyan}  Sentry Configuration Checker${colors.reset}`);
...
console.log(`${colors.bright}${colors.magenta}📋 Checking Configuration Files${colors.reset}\n`);
...
console.log(`${colors.bright}${colors.magenta}🔐 Checking Environment Variables${colors.reset}\n`);
...
requiredEnvVars.forEach(envVar => {
  ...
  if (value) {
    ...
    const maskedValue = envVar.includes('TOKEN') || envVar.includes('DSN')
...
console.log(`${colors.bright}${colors.magenta}⚙️  Checking Next.js Configuration${colors.reset}\n`);
...
try {
  ...
  if (fs.existsSync(nextConfigPath)) {
    ...
    if (nextConfig.includes('@sentry/nextjs') || nextConfig.includes('withSentryConfig')) {
      console.log(`${colors.green}✓${colors.reset} Sentry plugin configured in next.config.mjs`);
    } else {
      console.log(`${colors.yellow}⚠${colors.reset} Sentry plugin not found in next.config.mjs`);
    }

    if (nextConfig.includes('hideSourceMaps')) {
      console.log(`${colors.green}✓${colors.reset} Source maps are hidden in production`);
    } else {
      console.log(`${colors.yellow}⚠${colors.reset} Source maps hiding not configured`);
    }
  } else {
    console.log(`${colors.red}✗${colors.reset} next.config.mjs not found`);
  }
} catch (error) {
  console.log(`${colors.red}✗${colors.reset} Error reading next.config.mjs: ${error.message}`);
}

console.log();

// Provide Sentry dashboard links
console.log(`${colors.bright}${colors.magenta}🔗 Sentry Dashboard Links${colors.reset}\n`);
...
// Configuration tips
console.log(`${colors.bright}${colors.magenta}💡 Configuration Tips${colors.reset}\n`);
...
// Testing section
console.log(`${colors.bright}${colors.magenta}🧪 Testing Sentry Integration${colors.reset}\n`);
...
console.log(`   ${colors.cyan}throw new Error('Test Sentry Error');${colors.reset}\n`);

console.log(`${colors.yellow}2. Test API Error:${colors.reset}`);
console.log(`   In any API route:`);
console.log(`   ${colors.cyan}import * as Sentry from '@sentry/nextjs';${colors.reset}`);
console.log(`   ${colors.cyan}Sentry.captureException(new Error('API Test'));${colors.reset}\n`);

console.log(`${colors.yellow}3. Test Production:${colors.reset}`);
console.log(`   Visit: ${colors.cyan}https://your-app.vercel.app/api/test-sentry${colors.reset}`);
...
if (allFilesExist && allEnvVarsSet) {
  console.log(`${colors.green}${colors.bright}✓ Sentry is properly configured!${colors.reset}\n`);
} else {
  console.log(`${colors.yellow}${colors.bright}⚠ Sentry configuration incomplete${colors.reset}\n`);

  if (!allFilesExist) {
    console.log(`${colors.yellow}→ Some configuration files are missing${colors.reset}`);
  }
  if (!allEnvVarsSet) {
    console.log(`${colors.yellow}→ Some environment variables are not set${colors.reset}`);
  }
  console.log();
}

// Common commands
console.log(`${colors.bright}${colors.magenta}📝 Common Sentry Commands${colors.reset}\n`);
...
console.log(`${colors.bright}${colors.magenta}✅ Error Monitoring Checklist${colors.reset}\n`);
